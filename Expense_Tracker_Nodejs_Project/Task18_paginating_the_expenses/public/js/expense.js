...
const prevPageDiv = document.getElementById('prevPage');
...
function getAllExpenses(page=1, limit=5){
    ...
    .then((res) => {
        ...
        currentPageBtn.className = 'btn btn-outline-primary btn-sm w-100 rounded-0';
        currentPageBtn.addEventListener('click', () => getAllExpenses(currentPage, limit));

        const prevPageBtn = document.createElement('button');
        prevPageBtn.innerText = '<<Prev';
        prevPageBtn.className = 'btn btn-outline-primary btn-sm w-100 rounded-0';

        const nextPageBtn = document.createElement('button');
        nextPageBtn.innerText = 'Next>>';
        nextPageBtn.className = 'btn btn-outline-primary btn-sm w-100 rounded-0';
        ...
        currentPageDiv.appendChild(currentPageBtn);

        prevPageDiv.innerText = '';
        prevPageDiv.appendChild(prevPageBtn);

        nextPageDiv.innerText = '';
        nextPageDiv.appendChild(nextPageBtn);
    })
    .catch((err) => {
