...
function addExpense(e){ 
    ...
    const incomeRadio = document.getElementById('incomeRadio').checked;
    const expenseRadio = document.getElementById('expenseRadio').checked;

    let amount = parseInt(amountInput.value);
    if(incomeRadio === true){
        amount = Math.abs(amount);
    }else if(expenseRadio === true){
        amount = -Math.abs(amount);
    }else{
...
function showUserInfoInDOM(){
    ...
    if(isPremiumUser){
        ...
        premiumBtn.className = 'btn btn-outline-warning border-0';
...
function showDownloadedExpenseFileHistoryInDOM(){
    ...
    .then((res) => {
        ...
        const list = res.data.reverse(); console.log(list);
        downloadHistoryList.innerText = '';
        let rank = 1;
